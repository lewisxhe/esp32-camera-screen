...
extern uint8_t csccb_write(uint8_t addr, uint8_t reg, uint8_t data);
extern uint8_t csccb_read(uint8_t addr, uint8_t reg);
extern uint8_t csccb_probe();
...
{
#ifdef USE_CUSTOM
    return csccb_read(slv_addr, reg);
#else
    uint8_t data = 0;
...
{
#ifdef USE_CUSTOM
    return csccb_write(slv_addr, reg, data);
#else
    uint8_t ret = 0;
